 ...
 * https://www.youtube.com/watch?v=LHnJfVRyJ-c *boost sha*
 * https://api.libssh.org/stable/libssh_tutor_guided_tour.html
 * https://github.com/weniseb/RSA_CPP
 */
    std::string
    cifraMensagem(BD::dados &d, std::string mensagem = "") {
        if (mensagem.empty())
        ...
        for (size_t var = 0; var < mensagem.length(); ++var) {
            ...
            mensagemCifrada[var] ^=
                    chave[var % chave.size()]; // TODO trocar encriptacao xor por algo melhor
        }
        return mensagemCifrada;
    }

    std::string
    decifraMensagem(BD::dados &d, std::string mensagem = "") {
        if (mensagem.empty())
            mensagem = d.mensagem.simples;
        std::string mensagemCifrada =
